  ...
  return (
    <div className="calendar-days-wrapper">
      {weekDays.map((day, index) => {
        ...
        {intervalSlots.filter(slot => slot.day === day)
          .map((slot, slotIndex) => {
            return (
              <div
              className={
               ...
               key={slotIndex}
               data-interval={JSON.stringify({ hour: slot.hour, day: slot.day, currentDates: slot.currentDates })}
               onClick={() => this.bind(handleIntervalClick(slot))} 
              > </div>
            )
          })}
        </div>
