...
const CalendarDays = ({currentDates}) => {
  ...
  console.log(currentDates.forEach(element => {
    element
  }), 'THIS IS BEFORE useStete intervals');
  const [intervalSlots, setIntervalSlots] = useState(() => {
    let interval = [];
    weekDays.map((day, index) => {
      if (index !== 0) {
        for (let h of hours) {
          console.log(currentDates, 'this is IN THE LOOP');
          // console.log(currentDates[index], typeof(currentDates));
