      ...
      {weekDays.map((day, index) => {
        ...
        {intervalSlots.map((interval, index) => {
            ...
            return (
                ...
                <div className={interval.isOccupied ? "hour-interval-occupied" : "hour-interval-empty"} data-interval={JSON.stringify(interval)} onClick={handleIntervalClick} key={index} > </div>
            ) 
        })}
        </div>
        )
